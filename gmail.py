...
def fill_form(driver):
    try:
       ...
       # Create custom email
        ...
        if debug_mode:
            app_pass = input('Enter App password: ')
        out_save(f'{your_username}.@gmail.com',your_password,app_pass)
        driver.quit()
run_no,no_thread = sys.argv[1],sys.argv[2]

def multi_run(run_no, no_threads):
    threads = []
    ...
    for _ in range(no_threads):
        ...
        # Initialize a new driver instance for each thread to ensure independence
        service = ChromeService('chromedriver.exe')
        driver = webdriver.Chrome(options=chrome_options, service=service ) 
        
        # Pass the driver instance as a tuple
        thread = threading.Thread(target=lambda: fill_form(driver))
...
def main():
    multi_run(run_no=run_no,no_threads=no_thread)
